...
let arithmetic_puzzle l =
  ...
  in
  let rec to_string c l acc = match (c,l) with
    | ([], [x]) -> acc^(string_of_int x)
    | (hdc::tlc, hdl::tll) -> to_string tlc tll  ("("^acc^(string_of_int hdl)^")"^operator_string hdc)
  in
  let str = to_string (solve 0 ( (int_of_float (4.0**(float_of_int(n-2))))*(n-1)) (List.rev_append (List.rev_map (fun _ -> 1 ) (List.tl (List.tl l))) [0])) l ""
  in
  str
