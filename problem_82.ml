...
let cycles graph node = 
  let ns = neighbours node graph
  in
  List.map (fun x -> paths graph x node) ns |> List.flatten
  |> List.filter (fun x -> if List.length x<3 then false else true) |> List.map (fun x -> node::x) 
